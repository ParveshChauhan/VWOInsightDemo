...
extension MaterialShowcase {
    ...
    private func addBackground() {
        ...
        case .circle:
            ...
            if UIDevice.current.userInterfaceIdiom == .pad {
                backgroundView = UIView(frame: CGRect(x: 0, y: 150, width: radius * 1.7 ,height: radius * 1.7))
            } else {
    ...
    private func addTargetRipple(at center: CGPoint) {
        targetRippleView = UIView(frame: CGRect(x: 0, y: 0, width: targetView.bounds.width,height: targetView.bounds.height))
    ...
    private func addTargetHolder(at center: CGPoint) {
        ...
        targetHolderView = UIView(frame: CGRect(x: 0, y: 0, width: targetView.bounds.width,height: targetView.bounds.height))
        targetHolderView.center = center
        targetHolderView.backgroundColor = targetHolderColor
        //    targetHolderView.asCircle()
        hiddenTargetHolderView.frame = targetView.frame
    ...
    private func addTarget(at center: CGPoint) {
        var imag : UIImage?
        UIGraphicsBeginImageContextWithOptions(targetView.frame.size, false, 0)
        ...
        if let imag = imag {
            let img = UIImageView(image: imag)
            img.frame = CGRect(x: 0, y: 0, width: width, height: height)
            targetCopyView.addSubview(img)
        }
    ...
    private func addInstructionView(at center: CGPoint) {
        ...
        if UIDevice.current.userInterfaceIdiom == .pad {
            ...
            if UIDevice.current.userInterfaceIdiom == .pad {
                instructionView.frame = CGRect(x: 20,
                                               y: instructionView.frame.origin.y,
                ...
                //Updates horizontal parameters
                instructionView.frame = CGRect(x: 10,
                                               y: instructionView.frame.origin.y,
                                               width: UIScreen.main.bounds.width - 10 ,
                                               height: 0)
                instructionView.layoutIfNeeded()
