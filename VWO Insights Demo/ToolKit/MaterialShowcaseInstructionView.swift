...
public class MaterialShowcaseInstructionView: UIView {
  ...
  private func addSecondaryLabel() {
    ...
    secondaryLabel.frame = CGRect(x: 0,
                                  y: primaryLabel.frame.height+5,
                                  width: UIScreen.main.bounds.width - 10,
                                  height: 0)
    secondaryLabel.sizeToFitHeight()
    addSubview(secondaryLabel)
    frame = CGRect(x: frame.minX, y: frame.minY, width: UIScreen.main.bounds.width - 10, height: primaryLabel.frame.height + secondaryLabel.frame.height)
  }
